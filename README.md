...
- a bootable GPT disk image with:
  ...
  - read-only ext4 base root partitions for current and rollback boots
...
Base OS rollback slots are managed in the installed OS with `forgeos-rollback`. As root, create a rollback snapshot before changing the base OS:
...
If the current base fails later, reboot and choose `ForgeOS rollback selector` from GRUB. After booting the rollback slot and confirming it works, make it the current base again:

```bash
forgeos-rollback promote
```

If `forgeos-rollback create` says the source and destination are the same slot, check `forgeos-rollback status`. You are probably already booted from `root-rollback`; use `forgeos-rollback promote` from that boot to copy it back to `root`.
...
- The default GRUB menu boots `PARTLABEL=root` and includes a rollback selector for any additional base slots, starting with `PARTLABEL=root-rollback`. The primary selector entries use an initramfs-assisted fixed-framebuffer handoff so recovery boots stay visible.
- For default direct QEMU testing, `scripts/run-qemu.sh direct` uses the generated `rootfs.cpio.gz`.
- The host-side installer expands the writable app/state partition to fill larger target disks. Base root partitions stay fixed-size and read-only at runtime.
- Normal atomic images build with `ROLLBACK_SLOTS=2` by default: one current base slot plus one rollback base slot. Set `ROLLBACK_SLOTS=1` to use the old single-base-slot layout.
- `forgeos-rollback` can copy the currently booted read-only base into an inactive rollback slot and promote a booted rollback slot back to `root`.
...
Set `BOOTLOADER=stub` only when building a direct kernel EFI-stub image with its own firmware-provided load options or a kernel built with an embedded command line.
The rollback selector is generated by the default GRUB bootloader path.
...
- Nix is the package manager for the mutable app layer, but ForgeOS does not yet have a declarative NixOS-style base OS rebuild/update flow.
